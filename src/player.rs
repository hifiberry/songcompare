...
impl PlaybackState {
    ...
    fn fill_buffer(&mut self) {
        ...
        while self.ring_buffer_write_pos < self.ring_buffer_size
    ...
    fn seek(&mut self, offset: i64) {
        let new_pos = (self.position as i64 + offset
            - (self.ring_buffer_write_pos as i64 - self.ring_buffer_read_pos as i64))
            .max(0)
    ...
    fn get_position(&self) -> usize {
        ...
        self.position
            .saturating_sub(self.ring_buffer_write_pos - self.ring_buffer_read_pos)
    }
    ...
    fn switch_source(&mut self, new_samples: Vec<f32>, _new_sample_rate: u32) {
        ...
        println!(
            "Source switch initiated, will fade after {} samples over {} samples",
            PRE_FADE_PERIOD, self.fade_samples
        );
    }
}
...
impl Player {
    pub fn new(
        ...
        let matching_config = supported_configs.iter().find(|config| {
        ...
        if let Some(_config) = matching_config {
            ...
            println!(
                "Using: {} Hz, {} channels (NOTE: This may cause pitch/speed issues!)",
                actual_sample_rate, actual_channels
            );
        }
    ...
    pub fn switch_source(&self, new_samples: Vec<f32>, new_sample_rate: u32) {
        let _ = self
            .command_tx
            .send(PlayerCommand::SwitchSource(new_samples, new_sample_rate));
    }
    ...
    pub fn get_highest_sample_rate(channels: usize) -> Result<u32, Box<dyn std::error::Error>> {
        ...
        rates
            .last()
            .copied()
            .ok_or("No supported sample rates found".into())
    }
}
